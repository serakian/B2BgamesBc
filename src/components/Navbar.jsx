...
import { FaGamepad, FaChevronDown, FaSearch, FaUser, FaShoppingCart, FaSignInAlt, FaUserPlus } from 'react-icons/fa';
import { GiPistolGun, GiSwordman, GiPuzzle, GiCastle, GiRaceCar, GiBasketballBall, GiPlatform } from 'react-icons/gi';
...
const Navbar = ({ isAuthenticated, onLogout }) => {
  ...
  const genres = [
    { name: 'Action', icon: <GiPistolGun /> },
    { name: 'Adventure', icon: <GiSwordman /> },
    { name: 'Puzzle', icon: <GiPuzzle /> },
    { name: 'RPG', icon: <GiCastle /> },
    { name: 'Racing', icon: <GiRaceCar /> },
    { name: 'Sports', icon: <GiBasketballBall /> },
    { name: 'Platformer', icon: <GiPlatform /> }
  ];
    ...
    <nav className="luxe-navbar">
      <div className="luxe-nav-container">
        ...
        <div className={`luxe-nav-links ${isMobileMenuOpen ? 'open' : ''}`}>
          ...
          {/* Genres Dropdown */}
          <div 
            className="luxe-nav-dropdown"
            onMouseEnter={() => setIsGenreOpen(true)}
            onMouseLeave={() => setIsGenreOpen(false)}
          >
            <button className="luxe-dropdown-btn">
              Genres <FaChevronDown className={`luxe-chevron ${isGenreOpen ? 'open' : ''}`} />
            </button>
            {isGenreOpen && (
              <div className="luxe-dropdown-content">
                {genres.map((genre, index) => (
                  ...
                  >
                    <span className="luxe-genre-icon">{genre.icon}</span>
                    {genre.name}
                  </Link>
                ))}
              </div>
        ...
        <div className="luxe-nav-right">
          ...
          <button className="luxe-nav-icon-btn">
            <FaShoppingCart />
            <span className="luxe-cart-badge">3</span>
          </button>
        </div>
      </div>
    </nav>
